...
class Top_chi2_reconstructionProcessor(HggBaseProcessor):
    ...
    def process(self, events: awkward.Array) -> Dict[Any, Any]:
        ...
        for variation in variations:
            ...
            photons = photons[awkward.argsort(photons.mvaID, ascending=True)]
            #photons_max_MVAID = self.choose_nth_object_variable(photons.mvaID, -1, -999.0)
            photons_min_MVAID = self.choose_nth_object_variable(photons.mvaID, 0, -999.0)
            photons = photons[awkward.argsort(photons.mvaID, ascending=False)]
            photons_max_MVAID = self.choose_nth_object_variable(photons.mvaID, 0, -999.0)
            photons = photons[awkward.argsort(photons.pt, ascending=False)]
            photon_1_pt = self.choose_nth_object_variable(photons.pt, 0, -999.0)
            photon_1_eta = self.choose_nth_object_variable(photons.eta, 0, -999.0)
            photon_1_phi = self.choose_nth_object_variable(photons.phi, 0, -999.0)
            photon_2_pt = self.choose_nth_object_variable(photons.pt, 1, -999.0)
            photon_2_eta = self.choose_nth_object_variable(photons.eta, 1, -999.0)
            photon_2_phi = self.choose_nth_object_variable(photons.phi, 1, -999.0)
            ...
            diphotons["delta_phi"] = diphotons.pho_lead.delta_phi(diphotons.pho_sublead)
            diphotons["delta_r"] = diphotons.pho_lead.delta_r(diphotons.pho_sublead)
            ...
            diphotons_mass = self.choose_nth_object_variable(diphotons.mass, 0, -999.0)
            diphotons_pt = self.choose_nth_object_variable(diphotons.pt, 0, -999.0)
            
            #diphotons_delta_phi = self.choose_nth_object_variable(diphotons.delta_phi, 0, -999.0)
            #diphotons_delta_r = self.choose_nth_object_variable(diphotons.delta_r, 0, -999.0)
            ...
            event_mask = (((n_muons+n_electrons) >= 0) )
            ...
            first_jet_btagDeepFlav_B = choose_jet(jets.btagDeepFlav_B, 0, -999.0)
            ...
            second_jet_btagDeepFlav_B = choose_jet(jets.btagDeepFlav_B, 1, -999.0)

            first_jet_pt = numpy.where(w_mask, first_jet_pt, -999)
            first_jet_eta = numpy.where(w_mask, first_jet_eta, -999)
            first_jet_phi = numpy.where(w_mask, first_jet_phi, -999)
            first_jet_mass = numpy.where(w_mask, first_jet_mass, -999)
            first_jet_charge = numpy.where(w_mask, first_jet_charge, -999)
            first_jet_btagDeepFlav_B = numpy.where(w_mask, first_jet_btagDeepFlav_B, -999)

            second_jet_pt = numpy.where(w_mask, second_jet_pt, -999)
            second_jet_eta = numpy.where(w_mask, second_jet_eta, -999)
            second_jet_phi = numpy.where(w_mask, second_jet_phi, -999)
            second_jet_mass = numpy.where(w_mask, second_jet_mass, -999)
            second_jet_charge = numpy.where(w_mask, second_jet_charge, -999)
            second_jet_btagDeepFlav_B = numpy.where(w_mask, second_jet_btagDeepFlav_B, -999)
            ...
            diphotons["first_jet_btagDeepFlav_B"] = first_jet_btagDeepFlav_B
            ...
            diphotons["second_jet_btagDeepFlav_B"] = second_jet_btagDeepFlav_B

            diphotons["n_jets"] = n_jets
            diphotons["Njets2p5"] = Njets2p5

            #################Photons##################
            #photons_max_MVAID = numpy.where(w_mask,photons_max_MVAID,-999)
            #photons_min_MVAID = numpy.where(w_mask,photons_min_MVAID,-999)
            diphotons["photon_maxID"] = photons_max_MVAID
            diphotons["photon_minID"] = photons_min_MVAID
            ...
            def compute_chi2_modified(delta):
                v_matrix = numpy.array([[293.63,311.26],[311.26,756.77]],dtype = numpy.float64)
                delta_vector = numpy.array([delta["top"], delta["w"]], dtype=numpy.float64)
                v_inv = numpy.linalg.inv(v_matrix)
                chi2 = numpy.dot(delta_vector.T, numpy.dot(v_inv, delta_vector))
                return chi2
            ...
            for event_delta in (delta_vectors):
                event_chi2s = []
                for delta in (event_delta):
                    chi2 = compute_chi2_modified(delta)
...
############################################ Photons ############################################
            photon_1_pt_masked = numpy.where(w_mask, photon_1_pt, default_value)
            photon_1_eta_masked = numpy.where(w_mask, photon_1_eta, default_value)
            photon_1_phi_masked = numpy.where(w_mask, photon_1_phi, default_value)
            photon_2_pt_masked = numpy.where(w_mask, photon_2_pt, default_value)
            photon_2_eta_masked = numpy.where(w_mask, photon_2_eta, default_value)
            photon_2_phi_masked = numpy.where(w_mask, photon_2_phi, default_value)
            diphotons_mass_masked = numpy.where(w_mask, diphotons_mass, default_value)
            diphotons_pt_masked = numpy.where(w_mask, diphotons_pt, default_value)
            #diphotons_delta_phi_masked = numpy.where(w_mask, diphotons_delta_phi, default_value)
            #diphotons_delta_r_masked = numpy.where(w_mask, diphotons_delta_r, default_value)


            diphotons["pho_lead_pt"] = photon_1_pt_masked
            diphotons["pho_lead_eta"] = photon_1_eta_masked
            diphotons["pho_lead_phi"] = photon_1_phi_masked
            diphotons["pho_sublead_pt"] = photon_2_pt_masked
            diphotons["pho_sublead_eta"] = photon_2_eta_masked
            diphotons["pho_sublead_phi"] = photon_2_phi_masked
            diphotons["diphotons_mass_mask"] = diphotons_mass_masked
            diphotons["diphotons_pt_mask"] = diphotons_pt_masked
            #diphotons["diphotons_delta_phi_mask"] = diphotons_delta_phi_masked
            #diphotons["diphotons_delta_r_mask"] = diphotons_delta_r_masked
